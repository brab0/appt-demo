...
var UserRouter = exports.UserRouter = (_dec = (0, _core.ApptComponent)({
   extend: {
      type: _api.TRouter,
      config: {
         path: '/user'
   ...
   _createClass(UserRouter, [{
      ...
      value: function getAll(req, res, next) {
         this.controller.getAll().then(function (users) {
      ...
      value: function getById(req, res, next) {
         this.controller.getById(req.params.id).then(function (user) {
      ...
      value: function create(req, res, next) {
         this.controller.create(req.body).then(function (user) {
            return res.status(200).send(user);
         }).catch(function (ex) {
            return res.status(500).send(ex);
         });
      ...
      value: function update(req, res, next) {
         this.controller.update(req.body).then(function (user) {
            return res.status(200).send(user);
         }).catch(function (ex) {
            return res.status(500).send(ex);
         });
      ...
      value: function _delete(req, res, next) {
         this.controller.delete(req.params.id).then(function (user) {
            return res.status(200).send(user);
         }).catch(function (ex) {
            return res.status(500).send(ex);
         });
